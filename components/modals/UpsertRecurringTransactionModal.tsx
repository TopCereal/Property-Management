...
interface UpsertRecurringTransactionModalProps {
  properties: Property[];
...
const UpsertRecurringTransactionModal: React.FC<UpsertRecurringTransactionModalProps> = ({ properties, recurringTransaction, onClose, onSave }) => {
  const isEditMode = !!recurringTransaction;

  const [type, setType] = useState<TransactionType>(recurringTransaction?.type || 'Revenue');
  const [propertyId, setPropertyId] = useState<string | null>(recurringTransaction?.propertyId || null);
  const [description, setDescription] = useState(recurringTransaction?.description || '');
  const [category, setCategory] = useState(recurringTransaction?.category || (type === 'Revenue' ? REVENUE_CATEGORIES[0] : Object.values(EXPENSE_CATEGORIES)[0][0]));
  ...
  return (
    <BaseModal title={isEditMode ? 'Edit Recurring Transaction' : 'Add Recurring Transaction'} onClose={onClose}>
...
export default UpsertRecurringTransactionModal;