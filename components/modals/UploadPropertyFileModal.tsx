...
interface UploadPropertyFileModalProps {
  property: Property;
  onClose: () => void;
  onUpload: (fileData: Omit<PropertyFile, 'id'>) => void;
}

const UploadPropertyFileModal: React.FC<UploadPropertyFileModalProps> = ({ property, onClose, onUpload }) => {
  const [file, setFile] = useState<File | null>(null);
  const [category, setCategory] = useState<PropertyFileCategory>('Photos');
  const [subcategory, setSubcategory] = useState(PROPERTY_FILE_CATEGORIES['Photos'][0]);
  const [error, setError] = useState('');

  const subcategories = useMemo(() => PROPERTY_FILE_CATEGORIES[category] || [], [category]);

  const handleCategoryChange = (e: ChangeEvent<HTMLSelectElement>) => {
    const newCategory = e.target.value as PropertyFileCategory;
    setCategory(newCategory);
    setSubcategory(PROPERTY_FILE_CATEGORIES[newCategory][0]);
  };
  ...
  return (
    <BaseModal title={`Upload File for Lot #${property.lotNumber}`} onClose={onClose}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <select id="category" value={category} onChange={handleCategoryChange} className="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md">
                {Object.keys(PROPERTY_FILE_CATEGORIES).map(cat => <option key={cat} value={cat}>{cat}</option>)}
              </select>
            </div>
            <div>
...
export default UploadPropertyFileModal;